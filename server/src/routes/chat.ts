...
import { GroundedLlmService } from '../services/groundedLlm.service';
...
router.post('/ask', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const retrievalRes = await RetrievalService.search({
    ...
    const groundedResult = await GroundedLlmService.generateGroundedAnswer(question.trim(), retrievalRes);
 ...
 */
router.get('/boundary', (req: Request, res: Response) => {
  res.status(200).json({
    knows: [
      {
