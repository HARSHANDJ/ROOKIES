import { RetrievalResponse, SearchResultItem } from './retrieval.service';
import { ContradictionService, ContradictionAlert } from './contradiction.service';

export interface TrustMeterData {
  trustScore: number; // 0 to 100
  relevancePercent: string; // e.g. "98.4%"
  relevanceScore: number;
  evidenceStrength: 'Strong Evidence' | 'Moderate Evidence' | 'Insufficient Evidence';
  sourceCoverage: string; // e.g. "3 chunks across 2 documents"
  isGrounded: boolean;
}
...
export class GroundedLlmService {
  private static RELEVANCE_THRESHOLD = 0.50; // Minimum 50% similarity threshold
  ...
  public static async generateGroundedAnswer(
    ...
    const contradictionAlert = ContradictionService.detectContradiction(results);
    ...
    let evidenceStrength: 'Strong Evidence' | 'Moderate Evidence' | 'Insufficient Evidence' = 'Insufficient Evidence';
    if (hasSufficientEvidence) {
      evidenceStrength = maxScore >= 0.65 ? 'Strong Evidence' : 'Moderate Evidence';
    }

    const trustMeter: TrustMeterData = {
      trustScore,
      relevancePercent,
   ...
   */
  private static checkConceptAlignment(question: string, chunks: SearchResultItem[]): boolean {
    ...
    if (words.length === 0) return true; // generic question
...
Question: ${question}`,
            ...
            messages: [
              {
                role: 'system',
                content:
                  'You are KnowSphere AI. Answer ONLY using the supplied context. If evidence is missing, state: "I couldn\'t find enough information in the provided knowledge base to answer this accurately." Never guess.',
              },
              {
                role: 'user',
                content: `Context:\n${contextText}\n\nQuestion: ${question}`,
    ...
    for (const s of sentences) {
      ...
      for (const qw of qWords) {
        if (s.toLowerCase().includes(qw)) matches++;
      }
   ...
   */
  private static buildForensicsLineage(
    question: string,
    retrieval: RetrievalResponse,
    hasEvidence: boolean,
    answer: string
