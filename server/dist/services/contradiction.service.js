...
class ContradictionService {
    ...
    static detectContradiction(chunks) {
        ...
        const attendanceChunks = chunks.filter((c) => c.text.toLowerCase().includes('attendance') && c.text.includes('%'));
        if (attendanceChunks.length >= 2) {
            ...
            for (let i = 0; i < foundPercentages.length; i++) {
                for (let j = i + 1; j < foundPercentages.length; j++) {
                    ...
                    if (itemA.chunk.documentId !== itemB.chunk.documentId &&
        ...
        const feeChunks = chunks.filter((c) => (c.text.toLowerCase().includes('fine') || c.text.toLowerCase().includes('fee')) && c.text.includes('$'));
...
exports.ContradictionService = ContradictionService;
